...
const columns: DataTableColumns<Api.Core.Chat.Language.Model> = [
  ...
  { title: '模型名称', key: 'name', width: 180 },
  {
    ...
    render(row) {
      return h('span', { class: 'text-#6366f1 font-600' }, { default: () => row.cost });
    }
  },
  ...
  },
  {
    ...
    render(row) {
      return row.relatedUrl ? h('a', { href: row.relatedUrl, target: '_blank', class: 'text-#6366f1' }, row.relatedUrl) : '-';
    }
  },
  {
    title: '图片',
    ...
    width: 80,
    render(row) {
      return row.imgUrl ? h('a', { href: row.imgUrl, target: '_blank', class: 'text-#6366f1' }, '查看') : '-';
    }
  },
  {
    title: '操作',
...
const formValue = ref({
  data: { id: 0, name: '', model: '', cost: 0, relatedUrl: '', imgUrl: '', apiKey: '', baseUrl: '' }
});
const rules = {
  data: {
    name: { required: true, message: '模型名称不能为空', trigger: ['input', 'blur'] },
    model: { required: true, message: '模型型号不能为空', trigger: ['input', 'blur'] },
    cost: {
      required: true,
      trigger: ['input', 'blur'],
      validator(rule: FormItemRule, value: number) {
        if (!value && value !== 0) return new Error('模型价格不能为空');
        if (!/^[0-9]+$/.test(String(value))) return new Error('格式有误：请输入正整数');
        return true;
      }
    },
...
function openCreateDrawer() {
  isCreate.value = true;
  formValue.value.data = { id: 0, name: '', model: '', cost: 1, relatedUrl: '', imgUrl: '', apiKey: '', baseUrl: 'https://api.deepseek.com' };
  active.value = true;
}
...
function setDrawerDefaultData(row: Api.Core.Chat.Language.Model) {
  ...
  formValue.value.data = {
    id: row.id, name: row.name, model: row.model, cost: row.cost,
    relatedUrl: row.relatedUrl, imgUrl: row.imgUrl || '', apiKey: row.apiKey || '',
  ...
  try {
    ...
    if (isCreate.value) {
      const p: CreateModelParams = {
        name: formValue.value.data.name, model: formValue.value.data.model,
        cost: formValue.value.data.cost, relatedUrl: formValue.value.data.relatedUrl || '',
        imgUrl: formValue.value.data.imgUrl || '', apiKey: formValue.value.data.apiKey || '',
      ...
      const p: PostModelParams = { id: formValue.value.data.id, ...formValue.value.data };
      const index = data.value.findIndex(item => item.id === p.id);
      if (index !== -1) {
        data.value[index] = { ...data.value[index], ...p, updatedAt: dayjs().format('YYYY-MM-DD HH:mm:ss') };
      }
  ...
  <div>
    ...
    <NCard size="small" :bordered="true">
      <template #header>
        <div class="flex items-center justify-between">
          ...
          <NButton size="small" type="primary" @click="openCreateDrawer">新增模型</NButton>
        </div>
      </template>
      <NDataTable
        :columns="columns"
    ...
    <NDrawer v-model:show="active" :width="500" placement="right">
      <NDrawerContent :title="isCreate ? '新增模型' : '编辑模型'" closable>
        <template #footer><NButton type="primary" @click="drawerValidate">确认</NButton></template>
        <NForm ref="formRef" :label-width="90" :model="formValue" :rules="rules" size="medium">
          ...
          <NFormItem label="模型图片" path="data.imgUrl">
            <NInput v-model:value="formValue.data.imgUrl" placeholder="https://cdn.simpleicons.org/deepseek/1A1A1A" />
          </NFormItem>
          <NFormItem label="API Key" path="data.apiKey">
            <NInput v-model:value="formValue.data.apiKey" type="password" show-password-on="click" placeholder="请输入 API Key，留空将关闭该模型" />
          </NFormItem>
